 ...
 * @author min
 */
public class BubbleTeaListener implements ActionListener{
    
    private Manager mng;
    public BubbleTeaListener(Manager mng){
        this.mng = mng;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (clicked == mng.jButtonBackBubbleTea){
            mng.jPanelBubbleTeaView.setVisible(false);
            mng.jPanelMainView.setVisible(true);
            mng.jFrameMain.setSize(885, 650);
            mng.jFrameMain.getContentPane().setBackground(Color.WHITE);
        }else if (clicked == mng.jButtonPayWithQRCodeBubbleTeaView){
            mng.jPanelBubbleTeaView.setVisible(false);
            if (mng.checkCart == 1){
            ...
            //
        } else if (clicked == mng.jButtonPayInCashBubbleTeaView){
            mng.jPanelBubbleTeaView.setVisible(false);
            if (mng.checkCart == 1){
                mng.jFrameMain.add(mng.jPanelCartView);
                mng.checkCart=2;
            ...
            //
        } else if (clicked == mng.jButtonAddBubbleTeaView){
            long localSumMoneyBubbleTea=0;
            int localNumberBubbleTeaInBill=0;
            mng.bubbleTeaWriteBill="";
            //format  (String) "20,000" ->  (long) 20000
            DecimalFormat formatRemoveComma = new DecimalFormat("#,##0");
            Number number;
            try{
                for (int i = 0; i < mng.jTableMenuOfBubbleTea.getRowCount(); i++) {
                    if ((int)mng.jTableMenuOfBubbleTea.getValueAt(i, 3) != 0){
                        String numberString = (String) mng.jTableMenuOfBubbleTea.getValueAt(i, 2);
                        number = formatRemoveComma.parse(numberString);
                        long resultRemoveComma = number.longValue();
                        localSumMoneyBubbleTea+=resultRemoveComma*(int)mng.jTableMenuOfBubbleTea.getValueAt(i, 3);
                        
                        //duyet cho vao invoice
                        localNumberBubbleTeaInBill++;
                        //format a x b  c
                        DecimalFormat formatAddComma = new DecimalFormat("#,###");
                        String a = formatAddComma.format((int)mng.jTableMenuOfBubbleTea.getValueAt(i, 3));
                        String b = numberString;
                        String c = formatAddComma.format(resultRemoveComma*(int)mng.jTableMenuOfBubbleTea.getValueAt(i, 3));
                        ...
                        String nameFood =  (String) mng.jTableMenuOfBubbleTea.getValueAt(i, 1);
                        ...
                        mng.bubbleTeaWriteBill= mng.bubbleTeaWriteBill+"    "  + nameFood+"\t\t\t\n" 
            ...
            DecimalFormat formatAddComma = new DecimalFormat("#,###");
            String formattedNumberAddComma = formatAddComma.format(localSumMoneyBubbleTea);
            ...
            mng.jLabelMoneyBubbleTeaView.setText(formattedAddSpace);
            mng.sumMoneyBubbleTea =localSumMoneyBubbleTea;
            mng.numberBubbleTeaInBill = localNumberBubbleTeaInBill;
        }   else if ( clicked == jButtonFavoriteBubbleTeaView){
            mng.jTextAreaLikeUserView.setText(mng.jTextAreaLikeUserView.getText()+"- Tocotoco Bubble Tea - Vườn Lài\n");
